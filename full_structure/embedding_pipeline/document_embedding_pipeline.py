...
async def run_pipeline(req: DatasetRequest):
    ...
    async with httpx.AsyncClient(timeout=40.0) as client:
        ...
        step2 = await client.post(SERVICES_URL["CLEAN_STORED_DOCS"], params={"dataset_name": ds}, timeout=30)
        if step2.status_code != 200:
            return {"error": "cleaning failed", "detail": step2.text}
        print("STEP 2 done")

        print("running step 3 ")
        step3 = await client.post(SERVICES_URL["BUILD_EMBEDDING"], params={"dataset_name": ds}, timeout=50)
